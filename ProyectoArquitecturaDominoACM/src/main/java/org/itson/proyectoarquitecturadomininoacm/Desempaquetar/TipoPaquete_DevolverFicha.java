 ...
 * @author julio
 */
public class TipoPaquete_DevolverFicha implements IDesempaquetar{

    private IDesempaquetar siguiente;

    @Override
    public void manejarSolicitud(PaqueteDatos paqueteReciboDatos) {
        if (paqueteReciboDatos.getTipo() == TipoPaquete.DEVOLVER_FICHA) {
            FichaDTO fichaObtenida = (FichaDTO) paqueteReciboDatos.getObjeto();
            Ficha ficha = mediador.crearFichaDireccion(fichaObtenida);
